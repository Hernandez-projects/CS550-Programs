...
def graph_search(problem, verbose=False, debug=False):
    ...
    path = []
    stuffExplored = Explored()
    frontier.append(problem.state)

    # This block handles the root node creation
    nodes_explored = 1
    node = Node(problem, frontier.pop())
    stuffExplored.add(node.state)
    solution = (path, nodes_explored, t.elapsed())
    done = False
    ...
    while not done:
        if problem.goal_test(node.state):
            ...
            children = node.expand(node.problem)

        # this last part was supposed to handle the g and h functions to do the
        # appropriate search strategy but I did not understand how to implement it very much
